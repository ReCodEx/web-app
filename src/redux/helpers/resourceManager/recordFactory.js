...
import { resourceStatus, isReady, isReadyOrRealoading } from './status';
...
export const getData = resource =>
  isReadyOrRealoading(resource) ? resource.get('data') : null;
...
export const getJsData = resource => {
  const data = isReadyOrRealoading(resource) ? getData(resource) : null;
...
export const getId = resource =>
  isReadyOrRealoading(resource) ? getJsData(resource).id : null;
