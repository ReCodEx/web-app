...
export const addFile = createAction(actionTypes.UPLOAD_PENDING, wrapWithName, (id, file) => ({
  id,
  fileName: file.name,
}));
...
export const uploadFailed = createAction(actionTypes.UPLOAD_REJECTED, wrapWithName, (id, file) => ({
  id,
  fileName: file.name,
}));
...
const reducer = handleActions(
  {
    ...
    [actionTypes.REMOVE_FAILED_FILE]: (state, { payload, meta: { id } }) =>
      state.updateIn([id, 'failed'], list => list.filter(item => item.name !== payload.name)),
  },
  initialState
);
