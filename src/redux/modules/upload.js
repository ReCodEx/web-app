...
export const removeFailedFile = createAction(actionTypes.REMOVE_FAILED_FILE, (id, payload) => payload, (id) => ({ id }));
export const uploadSuccessful = createAction(actionTypes.UPLOAD_FULFILLED, (id, payload) => payload, (id) => ({ id }));
export const uploadFailed = createAction(actionTypes.UPLOAD_FAILED, wrapWithName, (id, file) => ({ id, fileName: file.name }));
...
const reducer = handleActions({
  ...
  [actionTypes.UPLOAD_PENDING]: (state, { payload, meta: { id, fileName } }) =>
    state
      ...
      .updateIn([ id, 'failed' ], list => list.filter(item => item.name !== payload.name))
      .updateIn([ id, 'removed' ], list => list.filter(item => item.name !== payload.name))
      .updateIn([ id, 'uploaded' ], list => list.filter(item => item.name !== payload.name)),
