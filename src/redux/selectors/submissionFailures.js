...
import { fetchManyEndpoint } from '../modules/submissionFailures.js';
...
export const submissionFailuresSelector = createSelector(getSubmissionFailures, getResources);
...
export const getSubmissionFailure = failureId =>
  createSelector(submissionFailuresSelector, failures => failures.get(failureId));

export const readySubmissionFailuresSelector = createSelector(submissionFailuresSelector, failures =>
  failures
    .toList()
    .filter(isReady)
    .map(getJsData)
