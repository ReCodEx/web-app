...
const prepareColumnDescriptors = lruMemoize(locale => {
  const columns = [
...
const MessagesList = ({ systemMessages, renderActions }) => {
  ...
  return (
    <>
      <FilterSystemMessagesForm
        ...
        initialValues={{ showAll }}
      />
      <hr className="m-0" />
      <SortableTable
        id="MessageList"
        hover
        columns={prepareColumnDescriptors(locale)}
        defaultOrder="visibleTo"
        data={prepareData(systemMessages, showAll, renderActions)}
        empty={
