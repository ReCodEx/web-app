...
const assignmentCellRendererCreator = defaultMemoize(
...
class ResultsTable extends Component {
  prepareColumnDescriptors = defaultMemoize(
    (assignments, shadowAssignments, loggedUser, locale) => {
      const {
        isAdmin,
        isSupervisor,
        links: {
  ...
  prepareData = defaultMemoize(
    (assignments, shadowAssignments, users, stats) => {
      const {
        isAdmin,
        isSupervisor,
        loggedUser,
      ...
      return users.map(user => {
        ...
        assignments.forEach(assignment => {
          data[assignment.id] = safeGet(
            userStats,
            ['assignments', a => a.id === assignment.id, 'points'],
            EMPTY_OBJ
          );
        });
