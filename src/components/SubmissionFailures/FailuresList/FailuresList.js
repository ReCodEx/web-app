...
const FailuresList = ({ failures, createActions }) => (
  <Table hover>
    <thead>
      <tr>
        <th>
          <FormattedMessage id="app.failureList.headType" defaultMessage="Type" />
        </th>
        <th>
        ...
        <th>
          <FormattedMessage id="app.failureList.headResolutionNote" defaultMessage="Resolution note" />
        </th>
        <th />
      </tr>
    </thead>
    <tbody>
      {failures.map((failure, i) => (
        <FailuresListItem id={failure.id} createActions={createActions} failure={failure} key={i} />
      ))}

      {failures.length === 0 && (
        <tr>
          <td className="text-center">
            <FormattedMessage id="app.failureList.noFailures" defaultMessage="There are no failures in this list." />
          </td>
        </tr>
      )}
    </tbody>
