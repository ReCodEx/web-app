...
describe('Resource manager', () => {
  describe('(Status)', () => {
    ...
    it('must determine if the record is too old or not', () => {
      ...
      expect(
        hundredMsIsTooOld(createRecord({ lastUpdate: Date.now() - 101 }))
      ).to.equal(true);
      expect(
        hundredMsIsTooOld(createRecord({ lastUpdate: Date.now() }))
      ).to.equal(false);
    });
    ...
    describe('default function to see if record is more than 10 minutes old', () => {
      it('must notice that a record is too old', () => {
        const oldRecord = createRecord({
          lastUpdate: Date.now() - (10 * 60 * 1000 + 5)
        }); // 10:00:005
      ...
      it('must leave a recent-enough record alone', () => {
        const okRecord = createRecord({
          lastUpdate: Date.now() - (9 * 60 * 1000 + 59 * 1000 + 998)
        }); // 9:59:998
